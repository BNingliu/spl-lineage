...
//
//public class PrestoSQLFieldLineageParserTest {
//    private SqlParserService parserService = new MyPrestoSQLParser();
//        @Test
//        public void insertQueryTest1() {
//            String sql = "INSERT INTO db_test.table_result(idd, name) SELECT t1.id, name_x FROM ( SELECT id1 + id2 AS id FROM db_test.table1 ) t1 LEFT OUTER JOIN ( SELECT id, name_x FROM ( SELECT id, sourcename AS name_x FROM db_test.table2 ) ) t2 ON t1.id=t2.id";
//
...
//                return null;
//            });
//            Assert.assertEquals(statementLineage.getQuerySql().get(), "INSERT INTO db_test.table_result(idd, name) SELECT t1.id, name_x FROM ( SELECT id1 + id2 AS id FROM db_test.table1 ) t1 LEFT OUTER JOIN ( SELECT id, name_x FROM ( SELECT id, sourcename AS name_x FROM db_test.table2 ) ) t2 ON t1.id=t2.id");
//        }
//
//    /*
//        @Test
