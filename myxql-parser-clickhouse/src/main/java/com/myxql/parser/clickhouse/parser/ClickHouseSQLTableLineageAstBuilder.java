...
public class ClickHouseSQLTableLineageAstBuilder extends CksqlBaseVisitor<StatementLineage> {
    private Integer dbType;

    public ClickHouseSQLTableLineageAstBuilder(Integer dbType) {
        this.dbType = dbType;
    }
    ...
    @Override
    public StatementLineage visitBlock(CksqlParser.BlockContext ctx) {
        System.out.println("visitBlock, ctx="+ctx.getText());
        if(ctx.stmt().size() > 1) {
            throw ClickHouseSQLTableLineageAstBuilder.parseError("不支持多个SQL的block", ctx);
        }
        StatementLineage data = visit(ctx.stmt(0));
        if (data == null) {
            throw ClickHouseSQLTableLineageAstBuilder.parseError("不支持的SQL: ", ctx);
        }
    ...
    @Override
    public StatementLineage visitStmt(CksqlParser.StmtContext ctx) {
    ...
    // ---------- database ------------------
    @Override
    public StatementLineage visitCreate_database_stmt(CksqlParser.Create_database_stmtContext ctx) {
        // 库名
        ...
        for(CksqlParser.Create_database_optionContext optionContext : ctx.create_database_option()) {
    ...
    @Override
    public StatementLineage visitDrop_database(CksqlParser.Drop_databaseContext ctx) {
        String databaseName = ctx.expr().getText();
    ...
    @Override
    public StatementLineage visitDrop_table(CksqlParser.Drop_tableContext ctx) {
        if(ctx.T_TABLE() != null) {
            ...
            throw ClickHouseSQLTableLineageAstBuilder.parseError("不不支持的删除语句", ctx);
        }
    }

    @Override
    public StatementLineage visitTruncate_stmt(CksqlParser.Truncate_stmtContext ctx) {
        TableName tableName = TableName.parseTableName(ctx.table_name().getText());
        Table table = new Table(tableName.getCatalogName(), tableName.getDatabaseName(), tableName.getTableName());
    ...
    // ---------- table ----------------------
    @Override
    public StatementLineage visitCreate_table_stmt(CksqlParser.Create_table_stmtContext ctx) {
        TableName tableName = TableName.parseTableName(ctx.table_name().getText());
        String fileFormat = null;

        List<CksqlParser.Create_table_preoptions_itemContext> preoptionsItems = Optional.ofNullable(ctx.create_table_preoptions())
                .map(CksqlParser.Create_table_preoptionsContext::create_table_preoptions_item)
                .orElse(new ArrayList<>());
        for(CksqlParser.Create_table_preoptions_itemContext preoptionsItem : preoptionsItems) {
            if(preoptionsItem.create_table_options_hive_item() != null) {
                CksqlParser.Create_table_options_hive_itemContext hiveItemContext = preoptionsItem.create_table_options_hive_item();
        ...
        if(ctx.create_table_definition() != null) {
            CksqlParser.Create_table_columnsContext columnsContext = ctx.create_table_definition().create_table_columns();
            ...
            List<CksqlParser.Create_table_options_itemContext> optionsItemContexts = Optional.ofNullable(ctx.create_table_definition().create_table_options())
                    .map(CksqlParser.Create_table_optionsContext::create_table_options_item)
                    .orElse(new ArrayList<>());

            for(CksqlParser.Create_table_options_itemContext optionsItem : optionsItemContexts) {
                if(optionsItem.create_table_options_hive_item() != null) {
                    CksqlParser.Create_table_options_hive_itemContext hiveItemContext = optionsItem.create_table_options_hive_item();
        ...
        CksqlParser.Create_table_definitionContext definitionContext = ctx.create_table_definition();
...
//            String querySql = StringUtils.substring(command, definitionContext.select_stmt().start.getStartIndex());
    ...
    @Override
    public StatementLineage visitDescribe_stmt(CksqlParser.Describe_stmtContext ctx) {
        TableName tableName = TableName.parseTableName(ctx.table_name().getText());
    ...
    // ---------- select ----------------------
    @Override
    public StatementLineage visitSelect_stmt(CksqlParser.Select_stmtContext ctx) {
    ...
    @Override
    public StatementLineage visitSubselect_stmt(CksqlParser.Subselect_stmtContext ctx) {
        // from table
        ...
        // join table
        List<CksqlParser.From_join_clauseContext> joinClauseContexts = Optional.ofNullable(ctx.from_clause())
                .map(CksqlParser.From_clauseContext::from_join_clause).orElse(new ArrayList<>());
        for(CksqlParser.From_join_clauseContext joinClauseContext: joinClauseContexts) {
    ...
    @Override
    public StatementLineage visitSelect_options_item(CksqlParser.Select_options_itemContext ctx) {
        if(ctx.T_LIMIT() != null) {
    ...
    @Override
    public StatementLineage visitInsert_stmt(CksqlParser.Insert_stmtContext ctx) {
        TableName tableName = TableName.parseTableName(ctx.table_name().getText());
        ...
        if(ctx.partitionSpec() != null) {
            List<CksqlParser.Partition_clauseContext> partitionClauseContexts = ctx.partitionSpec().partition_clause();
            for (CksqlParser.Partition_clauseContext partitionContext : partitionClauseContexts) {
        ...
        if(ctx.insert_stmt_rows() != null) {
            ...
            for(CksqlParser.Insert_stmt_rowContext rowContext : ctx.insert_stmt_rows().insert_stmt_row()) {
    ...
    private void parseCteTemplates(CksqlParser.Cte_select_stmtContext ctx) {
        this.isCTE = true;
        for(CksqlParser.Cte_select_stmt_itemContext itemContext : ctx.cte_select_stmt_item()) {
            this.currentOptType = StatementType.SELECT;
    ...
    @Override
    public StatementLineage visitCte_select_stmt_item(CksqlParser.Cte_select_stmt_itemContext ctx) {
        if (this.isCTE) {
