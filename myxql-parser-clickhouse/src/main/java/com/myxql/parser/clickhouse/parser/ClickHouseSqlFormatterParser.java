 ...
 */
public class ClickHouseSqlFormatterParser extends CksqlBaseVisitor {
    ...
    public ClickHouseSqlFormatterParser(String sql) {
        this.sourceSQL = sql;
    }
    ...
    private void makeWhereExpr(CksqlParser.Bool_exprContext bool_exprContext) {
    ...
    // select from 相关
    private String getFromTableClause(CksqlParser.From_table_clauseContext from_clauseContext, Integer kong) {
        ...
        Optional.of(from_clauseContext)
                ...
                .map(CksqlParser.From_subselect_clauseContext::T_OPEN_P)
        ...
        Optional.of(from_clauseContext)
                .map(CksqlParser.From_table_clauseContext::from_subselect_clause)
                .map(CksqlParser.From_subselect_clauseContext::T_CLOSE_P)
    ...
    //  select 字段相关
    private String getSelectItem(CksqlParser.Select_list_itemContext selectItem, Integer kong) {
        ...
        boolean isCase = Optional.of(selectItem)
                ...
                .map(CksqlParser.ExprContext::expr_case)
                .map(CksqlParser.Expr_caseContext::expr_case_searched)
                .map(expr_case_searchedContext -> {
                    StringBuilder tmpbuilder = new StringBuilder();
                    List<String> tmps = new ArrayList<>();
                    Optional.of(expr_case_searchedContext)
                            .map(CksqlParser.Expr_case_searchedContext::T_CASE)
                            .map(ParseTree -> addKongFormat(ParseTree.getText() + "\n", kong + 1))
                            .map(tmpbuilder::append);
                    Optional.of(expr_case_searchedContext)
                            .map(CksqlParser.Expr_case_searchedContext::T_WHEN)
                            .ifPresent(whenNodes -> {
                    ...
                    Optional.of(expr_case_searchedContext).map(CksqlParser.Expr_case_searchedContext::T_ELSE)
                            .map(ParseTree -> "\n" + addKongFormat(ParseTree.getText() +
                                    " " + expr_case_searchedContext.expr(expr_case_searchedContext.expr().size() - 1).getText(), kong + 2))
                            .map(tmpbuilder::append);
                    Optional.of(expr_case_searchedContext)
                            .map(CksqlParser.Expr_case_searchedContext::T_END)
    ...
    private String getFromJoin(CksqlParser.From_join_clauseContext ctx, int kong) {
        StringBuilder fromjoinSQL = new StringBuilder();
        Optional.of(ctx)
                .map(CksqlParser.From_join_clauseContext::from_join_type_clause)
        ...
        Optional.of(ctx)
                .map(CksqlParser.From_join_clauseContext::T_COMMA)
...
//                ).toArray(), ",\n")).map(fullSelect::append);
        ...
        for (int i = 0; i < selectSize; i++) {
            ...
            if (i < selectSize - 1) {
                CksqlParser.Fullselect_set_clauseContext clauseContext = ctx.fullselect_stmt().fullselect_set_clause(i);
        ...
        for (int i = 0; i < selectSize; i++) {
            CksqlParser.Subselect_stmtContext subSelect = ctx.fullselect_stmt().fullselect_stmt_item(i).subselect_stmt();
            ...
            tmpSelect.append(addKongFormat(
                    ...
                    kong)).append("\n");
            Optional.of(subSelect).map(CksqlParser.Subselect_stmtContext::select_list)
                    .map(CksqlParser.Select_listContext::select_list_set)
                    .map(select_list_setContext -> addKongFormat(subSourceSql(select_list_setContext) + "\n", kong + 1))
                    .map(tmpSelect::append);
            Optional.of(subSelect).map(CksqlParser.Subselect_stmtContext::select_list)
                    .map(CksqlParser.Select_listContext::select_list_item)
            ...
            Optional.of(subSelect)
                    ...
                    .map(CksqlParser.From_clauseContext::T_FROM)
...
//                    .map(tmpSelect::append);
            // where
            Optional.of(subSelect)
                    .map(CksqlParser.Subselect_stmtContext::where_clause)
                    .map(CksqlParser.Where_clauseContext::T_WHERE)
                    .map(ParseTree -> "\n" + addKongFormat(ParseTree.getText() + "\n", kong))
                    .ifPresent(tmpSelect::append);
            // where条件
            Optional.of(subSelect)
            ...
            Optional.of(subSelect)
                    ...
                    .ifPresent((tmpSelect::append));
            // order by 参数
            Optional.of(subSelect).map(CksqlParser.Subselect_stmtContext::order_by_clause)
                    .map(CksqlParser.Order_by_clauseContext::T_ASC)
                    .map(ParserRuleContext -> ParserRuleContext.size() > 0 ? "\n" + ParserRuleContext.get(0).getText() : "").ifPresent(tmpSelect::append);
            Optional.of(subSelect).map(CksqlParser.Subselect_stmtContext::order_by_clause)
                    .map(CksqlParser.Order_by_clauseContext::T_DESC)
                    .map(ParserRuleContext -> ParserRuleContext.size() > 0 ? "\n" + ParserRuleContext.get(0).getText() : "").ifPresent(tmpSelect::append);
            // select 参数
            Optional.of(subSelect).map(CksqlParser.Subselect_stmtContext::select_options)
