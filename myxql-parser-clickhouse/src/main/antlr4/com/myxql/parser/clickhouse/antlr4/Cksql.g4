...
create_table_options_hive_item :
     ...
     | T_STORED T_AS ident
     | T_COMMENT expr
     | T_PARTITIONED T_BY partitioning=partitionFieldList
     ;

partitionFieldList :
     T_OPEN_P fields+=partitionField (T_COMMA fields+=partitionField)* T_CLOSE_P
    ;

partitionField :
    ident (dtype)? (T_COMMENT expr)?
    ;
...
//     | ident ('%' (T_TYPE | T_ROWTYPE))?             // User-defined or derived data type
     | ident ('%' (T_ROWTYPE))?             // User-defined or derived data type, modify by myxql for remove type
     ;
...
drop_stmt :             // DROP statement
       T_DROP (T_TABLE | T_VIEW) (T_IF T_EXISTS)? table_name                   #drop_table
     | T_DROP T_PACKAGE (T_IF T_EXISTS)? ident                      #drop_package
     | T_DROP (T_PROCEDURE | T_FUNCTION) (T_IF T_EXISTS)? ident     #drop_function
     | T_DROP (T_DATABASE | T_SCHEMA) (T_IF T_EXISTS)? expr         #drop_database
     ;
...
insert_stmt :           // INSERT statement
       cte_select_stmt? T_INSERT (T_OVERWRITE T_TABLE | T_INTO T_TABLE?) table_name partitionSpec? insert_stmt_cols? (select_stmt | insert_stmt_rows)
     ;

partitionSpec :  // add by myxql
       T_PARTITION T_OPEN_P partition_clause (T_COMMA partition_clause)* T_CLOSE_P
     ;
partition_clause:
    ident (T_EQUAL expr)?
    ;
...
truncate_stmt :
       T_TRUNCATE T_TABLE? table_name partitionSpec?
     ;
...
group_by_clause :
       T_GROUP T_BY expr (T_COMMA expr)* ((T_WITH T_ROLLUP) | (T_WITH T_CUBE))?
     ;
...
expr_spec_func :
     ...
     | T_CURRENT_DATE (T_OPEN_P T_CLOSE_P)? | T_CURRENT T_DATE
     | T_CURRENT_TIMESTAMP (T_OPEN_P T_CLOSE_P)? | T_CURRENT T_TIMESTAMP
     | T_CURRENT T_TIMESTAMP (T_OPEN_P expr T_CLOSE_P)
     ...
     | T_COLLECT_SET T_OPEN_P expr T_CLOSE_P    // add by myxql
     | T_SYSDATE
     | T_USER
     ;
...
L_WS        : L_BLANK+ -> channel(HIDDEN) ;                                       // Whitespace, modify by myxql for rewirter
...
fragment
L_ID_PART  :
//             ([a-zA-Z] | '\u4E00'..'\u9FA5') ([a-zA-Z] | '\u4E00'..'\u9FA5' | L_DIGIT | '_' )*                         // Identifier part
             ([a-zA-Z] | '\u4E00'..'\u9FA5') ([a-zA-Z] | '\u4E00'..'\u9FA5' | L_DIGIT | '_' | '$' | '{' | '}')*          // Identifier part, modify by myxql for support ${} var
            | QuotedIdentifier                           // QuotedIdentifier, add by myxql
            | '$' '{' .*? '}'
